...
EVALUATORS = [
    epg.evaluate_bspline,
    epg.evaluate_bspline_matrix,
    epg.evaluate_bspline_vmap,
]
...
def test_basis_partition_and_scipy_consistency(evaluate):
    ...
    assert_allclose(basis.sum(axis=1), 1.0, atol=1e-14)
    assert np.all(basis >= 0.0)
    assert_allclose(
        evaluate(POINTS, KNOTS, COEFFICIENTS), expected @ COEFFICIENTS, atol=1e-14
    )
...
def test_fit_coefficient_count_and_reconstruction_on_actual_fit_grid():
    ...
    fitted = epg.fit_bspline_coefficients(jnp.asarray(samples), knots, 3)
    assert fitted.shape == (len(knots) - 3 - 1,)
    assert_allclose(fitted, COEFFICIENTS, atol=1e-12)
    assert_allclose(
        epg.evaluate_bspline(jnp.asarray(fit_points), knots, fitted),
        samples,
        atol=1e-12,
    )
...
@pytest.mark.xfail(
    reason="B01: experiment reconstruction grid differs from fit grid",
    raises=AssertionError,
)
def test_fit_reconstructs_on_experiment_sample_grid():
    knots = KNOTS.at[5:].set(8.01)
    samples = BSpline(np.asarray(knots), np.asarray(COEFFICIENTS), 3)(
        np.asarray(POINTS)
    )
    fitted = epg.fit_bspline_coefficients(jnp.asarray(samples), knots, 3)
    assert_allclose(epg.evaluate_bspline(POINTS, knots, fitted), samples, atol=1e-12)


@pytest.mark.parametrize("evaluate", EVALUATORS)
def test_left_boundary_and_right_hand_limit(evaluate):
    points = jnp.array([0.0, 8.0 - 1e-09])
    assert_allclose(
        evaluate(points, KNOTS, COEFFICIENTS),
        BSpline(np.asarray(KNOTS), np.asarray(COEFFICIENTS), 3)(points),
        atol=1e-14,
    )
...
def test_legacy_exact_right_boundary_is_excluded(evaluate):
    assert_allclose(evaluate(jnp.array([8.0]), KNOTS, COEFFICIENTS), [0.0], atol=0)


def test_spline_coefficient_derivative_and_jit():
    derivative = jax.jacobian(lambda c: epg.evaluate_bspline(POINTS, KNOTS, c))(
        COEFFICIENTS
    )
    expected = BSpline.design_matrix(np.asarray(POINTS), np.asarray(KNOTS), 3).toarray()
    assert_allclose(derivative, expected, atol=1e-14)
    compiled = epg.evaluate_bspline(POINTS, KNOTS, COEFFICIENTS)
    with jax.disable_jit():
        assert_allclose(
            epg.evaluate_bspline(POINTS, KNOTS, COEFFICIENTS), compiled, atol=1e-14
        )
