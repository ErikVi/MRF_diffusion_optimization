...
def evaluate_tensor_information(
    sequence, options, parameters, principal_direction, scales, sigma=10 ** (-1.65)
):
    ...
    if (
        ...
        raise ValueError("Invalid diagnostic coordinates/scales/noise")

    def signal(values):
        t1, t2, density, md, fa, object_phase = values
        tensor = axisymmetric_tensor(md, fa, principal_direction)
    ...
    arrays = dict(
        ...
        crlb=np.diag(covariance) * scales**2,
        principal_direction=np.asarray(principal_direction),
